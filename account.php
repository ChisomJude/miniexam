...
if (@$_GET['q'] == 34)
	echo 'class="active"';
?>><a href="account.php?q=34"><span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>&nbsp;Notes</a></li>	
	</ul>
...
<?php
if (@$_GET['q'] == 1) {
	$result = mysqli_query($con, "SELECT * FROM quiz WHERE status = 'enabled' AND class='$class' ORDER BY date DESC") or die('Error');
...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Total question</b></td><td style="vertical-align:middle"><b>Correct Answer</b></td><td style="vertical-align:middle"><b>Wrong Answer</b></td><td style="vertical-align:middle"><b>Total Marks</b></td><td style="vertical-align:middle"><b>Time limit</b></td><td style="vertical-align:middle"><b>Action</b></td></tr>';
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$time    = $row['time'];
		$eid     = $row['eid'];
		$q12 = mysqli_query($con, "SELECT score FROM history WHERE eid='$eid' AND examno='$username'") or die('Error98');
  ...
  <td style="vertical-align:middle"><b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="color:#FFFFFF;background:darkgreen;font-size:12px;padding:7px;padding-left:10px;padding-right:10px"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span><b>Start</b></span></a></b></td></tr>';
			...
			$q = mysqli_query($con, "SELECT * FROM history WHERE examno='$_SESSION[username]' AND eid='$eid' ") or die('Error197');
...
<br />';
			$q = mysqli_query($con, "SELECT * FROM user_answer WHERE qid='$qid' AND examno='$_SESSION[username]' AND eid='$_GET[eid]'") or die("Error222");
			if (mysqli_num_rows($q) > 0) {
			...
			$q = mysqli_query($con, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND examno='$_SESSION[username]'") or die("Error222a");
			$i = 1;
			while ($row = mysqli_fetch_array($q)) {
...
if (@$_GET['q'] == 'result' && @$_GET['eid']) {
	...
	$q = mysqli_query($con, "SELECT * FROM history WHERE eid='$eid' AND examno='$username' ") or die('Error157');
...
<center><h1 class="title" style="color:#660033">Result</h1><center><br /><table class="table table-striped title1" style="font-size:20px;font-weight:1000;">';
	...
	<tr style="color:darkblue"><td style="vertical-align:middle">Score&nbsp;<span class="glyphicon glyphicon-star" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $s . '</td></tr>';
		$q = mysqli_query($con, "SELECT * FROM rank WHERE  examno='$username' ") or die('Error157');
		while ($row = mysqli_fetch_array($q)) {
			$s = $row['score'];
		...
		while ($row = mysqli_fetch_array($q)) {
			$question = $row['qns'];
			$qid      = $row['qid'];
			$q2 = mysqli_query($con, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND qid='$qid' AND examno='$_SESSION[username]'") or die('Error197');
...
if (@$_GET['q'] == 2) {
	$q = mysqli_query($con, "SELECT * FROM history WHERE examno='$username' AND status='finished' ORDER BY date DESC ") or die('Error197');
...
if(@$_GET['q'] == 34 && !isset($_GET['n'] )) {
	 require_once 'student_view_note.php';
}
if(@$_GET['q'] == 34 && isset($_GET['n'] )) {
	$id = $_GET['n'];
